...
from dataset.dttd.dataset import DTTDDataset, Borderlist
...
def parse():
    ...
    parser.add_argument('--dataset_root', type=str, default = '../../datasets/dttd/DTTD_Dataset', help='dataset root dir')
...
def eval():
    ...
    # set model
    estimator = PoseNet(num_points = dataset.pt_num, num_obj = dataset.num_obj) # pt_num:1000, num_obj: 20
    estimator.cuda()
    ...
    for i in range(len(testlist)):
        ...
        for idx in range(len(lst)): # for each object in the scene
            itemid = lst[idx]
            try:
                data = process_data(itemid, img, depth, label, meta, Borderlist, num_points=dataset.pt_num, norm=dataset.norm)
                ...
                # add batch dim
                cloud = cloud.view(1, dataset.pt_num, 3)
                img_crop = img_crop.view(1, 3, img_crop.size()[1], img_crop.size()[2])
                
                # predict
                pred_r, pred_t, pred_c, _ = estimator(img_crop, cloud, sample, index)
                
                pred_r = pred_r / torch.norm(pred_r, dim=2).view(1, dataset.pt_num, 1)

                pred_c = pred_c.view(1, dataset.pt_num)
                pred_t = pred_t.view(1 * dataset.pt_num, 1, 3)
                points = cloud.view(1 * dataset.pt_num, 1, 3)
