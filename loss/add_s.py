...
def loss(pred_r, pred_t, pred_c, model_target, model, idx, point_cloud, w, num_point_mesh, knn, sym_list):
    ...
    # if the current object is symmetrical, find the nearest point
    # TODO: batch size should be restricted to 1
    # TODO: Should run on CUDA environment, CPU is not supported
    if idx[0].item() in sym_list:
        model_target = model_target[0].contiguous().view(-1, 3)
        pred = pred.contiguous().view(-1, 3)
        _, inds = knn(model_target.unsqueeze(0), pred.unsqueeze(0))
        model_target = torch.index_select(model_target, 0, inds.view(-1))
        model_target = model_target.view(bs*num_p, num_point_mesh, 3)
        pred = pred.view(bs*num_p, num_point_mesh, 3)
    
    # get distance and loss
    dis = torch.mean(torch.norm((pred - model_target), dim=2), dim=1)
...
class ADDS_Loss(_Loss):

    def __init__(self, num_points_mesh, sym_list):
        super(ADDS_Loss, self).__init__(True)
        self.num_pt_mesh = num_points_mesh
        self.sym_list = sym_list
        self.knn = KNN(k=1, transpose_mode=True)

    def forward(self, pred_r, pred_t, pred_c, target, model_points, idx, points, w):
        return loss(pred_r, pred_t, pred_c, target, model_points, idx, points, w, self.num_pt_mesh, self.knn, self.sym_list)